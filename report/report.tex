...
\begin{enumerate}
\item Hardware Probleme - Im Fall eines einzelnen Hardware-Ausfalls (z.B. CPU) sollen Benutzer keinen Unterbruch wahrnehmen
\item Applikations- Code Probleme - Mögliche Errors werden abgefangen so dass das Spiel nicht crasht
\item Dependency probleme - Benutzte externe Libraries werden auf dem neusten Stand gehalten und der Code entsprechend angepasst.
\end{enumerate}

\end{itemize}

Um die Qualitätsattribute einzuhalten, ist die Sicherstellung von Entwicklungsprinzipien (z.B. bzgl. Code) und Eskalationsplänen ein Muss. Diese sind dank agilen Framework-Prozessen wie Scrum und zuverlässigen Entwicklungstools einfacher einzuhalten.
...
Um die Qualität der Software bei der Entwicklung unseres Spieles
...
Der Aufbau unserer Software und in welchen Schritten und Phasen entwickelt wird, wurde im Voraus in einer Grobplanung definiert.
...
\end{figure}
%\includegraphics[width=0.7\textwidth]{./img/Intent.svg}
% Vielleicht später die Recording Engine & Co.?

\section{Data}
% In den bisherigen Sektionen war der Fokus nicht primär auf die Daten gerichtet, obwohl diese in modernen Systemen eine essentielle Rolle spielen. Ziel dieser Sektion ist die Informationen zu dokumentieren, die aus Datenperspektive wichtig sind.Auch hier soll jede Sektion kurz gehalten werden. Eventuell ein Entity-Relationship Diagramm, wenn es dem Leser hilft.
