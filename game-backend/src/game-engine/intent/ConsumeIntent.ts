...
export default class ConsumeIntent implements Intent {
  private heal(health: number): number {
    return Math.min(health + FOOD_REGENERATION_VALUE, MAX_HEALTH);
  }

  private getConsumableInReach(state: State, unit: Unit): Food | null {
  ...
  execute(state: State, userContext: UserContext): State {
    return produce(state, draft => {
      ...
      const consumable = this.getConsumableInReach(state, unit);
      const canConsume = !!consumable;

      if (canConsume) {
        unit.health = this.heal(unit.health);
